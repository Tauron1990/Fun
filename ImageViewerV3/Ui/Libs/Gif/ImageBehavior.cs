...
using ImageViewerV3.Ui.Libs.Gif.Decoding;
...
namespace ImageViewerV3.Ui.Libs.Gif
{
    ...
    public static class ImageBehavior
    {
        ...
        private static void AnimateInDesignModeChanged(DependencyObject o, DependencyPropertyChangedEventArgs e)
        {
            ...
            var source = GetAnimatedSource(imageControl);
            if (source == null || !imageControl.IsLoaded) return;
            if (newValue)
                InitAnimationOrImage(imageControl);
            else
        ...
        private static ObjectAnimationUsingKeyFrames? GetAnimation(Image imageControl, BitmapSource source)
        {
            ...
            if (animation != null)
                return animation;
            if (!(GetDecoder(source, imageControl, out var gifMetadata) is GifBitmapDecoder decoder) || decoder.Frames.Count <= 1) return null;
        ...
        private static BitmapDecoder? GetDecoder(BitmapSource image, Image imageControl, out GifFile? gifFile)
        {
            ...
            switch (decoder)
            {
                case GifBitmapDecoder _ when !CanReadNativeMetadata(decoder):
                {
                    if (stream != null)
                    {
                        stream.Position = 0;
        ...
        private static RepeatBehavior GetActualRepeatBehavior(Image imageControl, BitmapDecoder decoder, GifFile? gifMetadata)
        {
            ...
            return repeatCount == 0 ? RepeatBehavior.Forever : new RepeatBehavior(repeatCount);
        }
        ...
        private static FrameMetadata GetFrameMetadata(GifFrame gifMetadata)
        {
            ...
            if (gce == null) return frameMetadata;
            if (gce.Delay != 0)
